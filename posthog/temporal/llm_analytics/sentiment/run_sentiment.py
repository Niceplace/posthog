...
async def classify_sentiment_activity(input: SentimentClassificationInput) -> dict[str, Any]:
    ...
    # Max per-message scores for positive/negative so the UI can show range markers
    positive_max_score = max(r.scores.get("positive", 0.0) for r in classify_results)
    negative_max_score = max(r.scores.get("negative", 0.0) for r in classify_results)
